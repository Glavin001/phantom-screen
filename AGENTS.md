...
- **WebCodecs H.264 must use software decoding**: The Cloud VM has no GPU, so `hardwareAcceleration: 'prefer-hardware'` fails with "Unsupported configuration". Use `'prefer-software'` or `'no-preference'` instead. The codec `avc1.42001f` works fine with software decoding.
